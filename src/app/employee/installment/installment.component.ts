...
@Component({
  ...
  styleUrls: ['./installment.component.sass']
})
export class InstallmentComponent implements OnInit {
  ...
  columns = [
    { name: 'polcode' }, { name: 'poldate' }, { name: 'custname' }, { name: 'ins' }, { name: 'payIns' }, { name: 'inssStatus' }
  ];

  allColumns = [{ name: 'polcode' }, { name: 'poldate' }, { name: 'custname' }, { name: 'ins' }, { name: 'payIns' }, { name: 'inssStatus' }];
  ...
  constructor(private dynamicScriptLoader: DynamicScriptLoaderService, private fb: FormBuilder, public authService: ApiServiceProvider) {
  ...
  editRow(row) {
    ...
    this.basicForm.patchValue({
      ...
      ins: row.ins,
      payIns: row.payIns,
      inssStatus: row.inssStatus,
    });
    this.selectedRowData = row;

    M.updateTextFields();

    this.authService.postData({"data":{"pol_code":row.polcode}}, "installment/get").then((result) => {
      this.resposeData =result;
      
      if(this.resposeData.data.length > 0){
        this.selectedData = this.resposeData.data;
        console.log(this.selectedData);
        
      }else{
        swal("ไม่พบข้อมูล","เตือน","wraning")
      }
    }, (err) => {
      swal("ผิดพลาด","","danger");
    });
  ...
  deleteRow(row) {
    // console.log(row.id);
    this.data = this.arrayRemove(this.data, row.polcode)
    this.showNotification("bg-red", "Delete "+row.polcode+" Record Successfully ", "bottom", "right", "animated fadeInRight", "animated fadeOutRight")
  }

  arrayRemove(array, polcode) {
    return array.filter(function (element) {
      return element.polcode != polcode;
    });
  ...
  listData(){
    this.authService.postData({}, "installment/list").then((result) => {
      this.resposeData =result;
      this.data = this.resposeData.data;
    }, (err) => {
      swal("ผิดพลาด","","danger");
    });
  }
  ...
  getData(pol){
    this.authService.postData({data:{pol_code:pol}}, "installment/get").then((result) => {
      this.resposeData = result;
